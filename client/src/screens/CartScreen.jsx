      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-12">
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-20">
            <FaShoppingBag className="w-16 h-16 text-luxe-charcoal/30 mx-auto mb-6" />
            <h2 className="font-serif text-3xl font-light text-luxe-black mb-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {cartItems.map((item) => (
                <div
                  key={item._id}
                  className="bg-luxe-warm-white border border-luxe-charcoal/10 p-6 flex flex-col md:flex-row gap-6 group hover:border-luxe-gold/30 transition-all duration-300"
                >
                  {/* Product Image */}
                  <Link
                    to={`/product/${item._id}`}
                    className="flex-shrink-0 w-32 h-32 md:w-40 md:h-40 overflow-hidden bg-luxe-cream"
                  >
                    <img
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <Link
                        to={`/product/${item._id}`}
                        className="font-serif text-xl font-normal text-luxe-black hover:text-luxe-gold transition-colors duration-200 mb-2"
                      >
                        {item.name}
                      </Link>
                      {item.brand && (
                        <p className="font-sans text-xs uppercase tracking-wider text-luxe-charcoal/60 mb-4">
                          {item.brand}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3 border border-luxe-charcoal/20">
                        ...
                        <span className="font-sans text-base w-12 text-center">{item.quantity}</span>
                        <button
                          onClick={() =>
                            updateCartHandler(
                              item,
                      ...
                      <div className="text-right">
                        <p className="font-serif text-xl font-normal text-luxe-black">
                      ...
                      <button
                        ...
                        className="p-2 text-luxe-charcoal/50 hover:text-luxe-gold transition-colors duration-200"
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 bg-luxe-warm-white border border-luxe-charcoal/10 p-6">
                ...
                <Link
                  to="/products"
                  className="block text-center mt-4 font-sans text-sm text-luxe-charcoal/70 hover:text-luxe-gold transition-colors duration-200"
                >
